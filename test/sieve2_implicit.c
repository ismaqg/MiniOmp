...
#pragma omp parallel //reduction(+: found)
{
  ...
  for(int from = 2 + id*sliceSize; from <= lastNumber; from += numthreads*sliceSize){ // solo ha hecho falta cambiar la cabecera del for :)
        /* estas 2 lineas de abajo son equivalentes a: to = min(from+sliceSize, lastNumber)*/
	int to = from + sliceSize;
        if (to > lastNumber) to = lastNumber; 
        #pragma omp atomic //-> Sustituimos el atomic por reduction(+: found) en el parallel
